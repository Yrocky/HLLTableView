...
@property (nonatomic ,strong) NSArray * sectionObjArray;
...
- (void)viewDidLoad {
    ...
    // setter data
    _sectionObjArray = [self.manager knownRegions];
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    if (self.sectionInfoArray == nil ||
        ...
        for (HLLRegion * region in self.sectionObjArray) {
...
- (NSInteger) tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    NSInteger numberStoriesSection = sectionInfo.region.timeZones.count;

    return sectionInfo.isOpen ? numberStoriesSection : 0;
}

- (NSInteger) numberOfSectionsInTableView:(UITableView *)tableView{
    
    return self.sectionObjArray.count;
}
